...
public class MethodBodyReader
{
    ...
    {        
        int h=((il[position++] | (il[position++] << 8)) | (il[position++] << 0x10)) | (il[position++] << 0x18);
        return (h);
    }
        ...
        while (position < il.Length)
        {
            ...
            switch (code.OperandType)
            {
                ...
                case OperandType.InlineString:
                    {
                        metadataToken = ReadInt32(il, ref position);
                        instruction.Operand = module.ResolveString(metadataToken) + metadataToken.ToString();
                        break;
                    }
    ...
    public string GetBodyCode(byte[] IL)
    {
        string Res = "";
        string s = ""; int k;
        if (instructions != null)
        {
            for (int i = 0; i < instructions.Count; i++)
            {
                k = (i == instructions.Count - 1) ? (IL.Length - instructions[i].Offset - 1) : 
                    (instructions[i + 1].Offset - instructions[i].Offset - 1);
                s = "  (" + IL[instructions[i].Offset];//I'm not sure if there is join in net 2.0                 
                var bInstruction = Addition.ReadBytes(IL, instructions[i].Offset + 1, k);

                foreach (var b in bInstruction) s = s + "|" + b.ToString(); s = s + ") ";

                if (k == 4)
                {
                    s = s + Addition.ToInt32(bInstruction).ToString();
                 // var test0=RealTokens.module.ResolveMethod(AdditAOP.ToInt32(ww));                   
                    Res += instructions[i].GetCode() + s + "\n";
                }
                else
                    Res += instructions[i].GetCode() + s + "\n";
            }
    ...
    public MethodBodyReader(Module m, byte[] ill)
    {
        this.il = ill;
        ConstructInstructions(m);
    }
}
